...
"""
    n -> size of the program's input
        factorial (n!) -> number we want to calculate factorial for
        fibonacci -> index of the term we want to compute
        merge sort -> the length of the list

    BC(n) -> how the input data organized (e.g. list already sorted)
    AC(n)
    WC(n) -> how the input data organized (e.g. list sorted in reverse)
...
"""
 ...
 ....
 n. fin elem on pos n-1 -> n

 assume P(I) is uniform distribution

 T(n) = (1 + 2 + 3 + ... + n)/n =  n * (n+1)/ (2*n) in T(n)
"""


# T(n) = 10*n in O(n)
def f_1(n: int):  # n represents the size of the program's input
...
# for n = 3000, we expect runtime of 3^4=81 ms to complete
def f_5(n: int):
...
# m doesn't contribute to complexity
# T(n) = 2*n, O(n)
def f_8(n, m: int):
    for i in range(0, n):  # n loops
        print("Hello World")
    for j in range(0, n):  # n loops
        print("Hello World")
...
extra-space complexity 
    T(n) = 2 * T(n/2) + 2 * n/2
         ...
         just like above, but all term get multiplied by n => O(n^2)   
...
"""
we nee dto look at n to determine the complexity
time complexity 
    T(n) = 1, n<2
    T(n) = 4 * T(n/2) + 1
    T(n/2) = 4 * T(n/4) + 1
...
"""
slicing in python 
    work for Sequence types -- str, list
"""
...
print(data[2:])  # until de end of the list
print(data[:4])  # from the beginning
print(data[:-1])  # print everything except the last element
print(data[::2])  # din 2 in 2
print(data[:5:3])