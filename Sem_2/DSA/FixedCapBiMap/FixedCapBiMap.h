...
class FixedCapBiMap {
    ...
    // For key-value pair <Key, old_value> replace the old_value with the new_value
    // return true if replacement done successfully
    // return false if the pair is not in the array
    bool replace(TKey key, TValue old_value, TValue new_value);
