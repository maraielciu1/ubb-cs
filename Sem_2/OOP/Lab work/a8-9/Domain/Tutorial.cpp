...
std::ostream &operator<<(std::ostream &os, const Tutorial &tutorial) {
    os << tutorial.getTitle() << "," << tutorial.getPresenter() << "," << tutorial.getLink() << "," << tutorial.getMinutes() << ":" << tutorial.getSeconds() << "," << tutorial.getLikes();
    return os;
}
...
std::istream &operator>>(std::istream &is, Tutorial &tutorial) {
    ...
    if (tokens.size() != 5) // make sure all the tutorial data was valid
        return is;

    tutorial.title = tokens[0];
    tutorial.presenter = tokens[1];
    tutorial.link = tokens[2];

    // get duration
    std::vector<std::string> durationTokens = tokenize(tokens[3], ':');
    if (durationTokens.size() != 2)
        return is;
    tutorial.minutes=stod(durationTokens[0]);
    tutorial.seconds=stod(durationTokens[1]);

    tutorial.likes=stod(tokens[4]);
