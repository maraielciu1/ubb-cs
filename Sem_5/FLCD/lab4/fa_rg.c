...
void fa_print(FiniteAutomata* fa) {
    ...
    printf("  Alphabet: {");
    for (int i = 0; i < fa->num_alphabet; i++) {
        printf("%s", fa->alphabet[i]);
        if (i < fa->num_alphabet - 1) printf(", ");
    }
    printf("}\n");
...
void rg_print(RegularGrammar* rg) {
    printf("Regular Grammar (Right Linear):\n");
    
    printf("  Non-terminals: {");
    for (int i = 0; i < rg->num_non_terminals; i++) {
        printf("%s", rg->non_terminals[i]);
        if (i < rg->num_non_terminals - 1) printf(", ");
    }
    printf("}\n");
    
    printf("  Terminals: {");
    for (int i = 0; i < rg->num_terminals; i++) {
        printf("%s", rg->terminals[i]);
    ...
    for (int i = 0; i < fa->num_final_states; i++) {
        ...
        for (int j = 0; j < fa->num_states; j++) {
            if (strcmp(fa->states[j], fa->final_states[i]) == 0) {
                ...
                for (int k = 0; k < rg->num_production_rules; k++) {
                    if (strcmp(rg->productions[k].non_terminal, final_nt) == 0) {
                        for (int l = 0; l < rg->productions[k].num_productions; l++) {
                            if (strcmp(rg->productions[k].productions[l], "ε") == 0 ||
                ...
                if (!has_epsilon) {
                    rg_add_production(rg, final_nt, "ε");
                }
                break;
            }
        }
...
FiniteAutomata* rg_to_fa(RegularGrammar* rg) {
    ...
    for (int i = 0; i < rg->num_production_rules; i++) {
        ...
        if (from_state_idx == -1) continue;
        
        for (int j = 0; j < rg->productions[i].num_productions; j++) {
            const char* rhs = rg->productions[i].productions[j];
            
            // Handle epsilon
            if (strcmp(rhs, "ε") == 0 || strcmp(rhs, "epsilon") == 0) {
...
bool fa_verify(FiniteAutomata* fa, const char* input) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        // Special handling for integer constants: first digit after sign or start must be nonzero
        if (strcmp(symbol, "digit") == 0 && input[i] == '0') {
            // This is '0', but map_char_to_symbol returned "digit" for it
            // Check if we're at start or after sign - if so, use "0" instead
            if (i == 0 || (i == 1 && (input[0] == '+' || input[0] == '-'))) {
                symbol = "0";
            }
        }
        ...
        // Special check: if we're in q3 (final state for "0") and there are more characters, reject
        if (strcmp(current_state, "q3") == 0 && i < len - 1) {
            return false; // "0" followed by more digits is invalid (e.g., "01")
        }
    }
