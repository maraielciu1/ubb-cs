...
public:
    ...
    // For big number support (bonus)
    void setBigNumberMode(bool enabled);
    bool isBigNumberMode() const;
    
private:
    std::vector<int> coefficients_;
    bool big_number_mode_ = false;
};

// Regular O(n²) sequential multiplication
Polynomial multiplyRegularSequential(const Polynomial& a, const Polynomial& b);

// Regular O(n²) parallel multiplication
