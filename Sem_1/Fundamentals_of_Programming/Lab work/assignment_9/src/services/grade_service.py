from src.actions.add.grade import AddGradeCommand
...
class GradeService:
    ...
    def add_grade(self, grade_id, student_id, discipline_id, value):
        ...
        try:
            ...
            return response_data
        try:
            grade = Grade(grade_id, grade_student, grade_discipline, value)
        except TypeError as te:
            response_data["success"] = False
            response_data["message"] = str(te)
            return response_data
        try:
            add_grade_command = AddGradeCommand(self.repository, grade)
            add_grade_command.execute()
            self.repository.record_undo_commands([add_grade_command])
        except RepositoryError as re:
            response_data["success"] = False
            response_data["message"] = str(re)
            return response_data
    ...
    def remove_grade(self, grade_id):
        response_data = {"success": True,
                         "message": f"Grade with ID {grade_id} removed successfully!"}
        try:
            remove_grade_command = RemoveGradeCommand(
                self.repository, grade_id)
            remove_grade_command.execute()
            self.repository.record_undo_commands([remove_grade_command])
    ...
    def remove_grades_by_student(self, student_id):
        ...
        try:
            grades_to_remove = self.repository.get_grades_by_student(
                student_id)
            undo_commands = []
            for grade in grades_to_remove:
                remove_grade_command = RemoveGradeCommand(
                    self.repository, grade.id)
                remove_grade_command.execute()
                undo_commands.append(remove_grade_command)
            self.repository.record_undo_commands(undo_commands)
    ...
    def remove_grades_by_discipline(self, discipline_id):
        ...
        try:
            grades_to_remove = self.repository.get_grades_by_discipline(
                discipline_id)
            undo_commands = []
            for grade in grades_to_remove:
                remove_grade_command = RemoveGradeCommand(
